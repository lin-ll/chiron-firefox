...
function insertDiv(child) {
  ...
  div.setAttribute('style', 'position: fixed; top: 1em; right: 1em; padding: 10px 20px; '
    +'border: 1px solid #007095; border-radius: 2em; max-width: 34em; max-height: 40em; '
...
function parseAjax(word, toReturn) {
  ...
  if (resultFound) {
    ...
    insertDiv('<div id="paideia-panel"><button id="remove" style="float: right;">X</button>' + header + "<br /><h4>Possible Forms:</h4>" + table + anotherDictionary(word) + thanks + '</div>');
    $('#remove').click(rmPanel);
  } 
